...
function stopReasonError(reason, text) {
  if (reason === void 0) {
    // Strict: a run that never emitted turn/end is not a success, even if it
    // produced some text along the way (review feedback: require the terminal marker).
    return text === ""
      ? "child run produced no turn/end outcome"
...
export function apply(ctx) {
  const dispose = ctx.tools.register(defineTool({
    name: "preset_run",
    description: "Create a fresh, independent agent session composed from the named agent preset (exactly like the Web UI's New Session flow, but headless), send the given task as its first user message, wait for it to finish, and return the final answer text. The child session is routed with the deployment's agent-default-model (settings.yaml) and gets the preset's own tools, prompt sections, and skills. It is torn down afterwards. Use this to run a one-shot task under a specific agent preset (e.g. router-spec / router-standard / standard / minimal / cordis) without touching the current conversation. When the optional `sessionId` is given, the task is instead appended as the NEXT user message of that existing session (same preset must be passed), i.e. the conversation continues like a chat and the previous context is kept.",
    parameters: {
      preset: {
        type: "string",
        required: true,
        description: "Agent preset id to compose the child session from, e.g. router-spec, router-standard, standard, minimal or cordis. It must exist in the agent-presets roster. When resuming (sessionId given), this must be the same preset the session was created with."
      },
      task: {
        type: "string",
        required: true,
        description: "The task text to send as the child session's first (or, when resuming, next) user message."
      },
      sessionId: {
        type: "string",
        description: `Optional session id (e.g. "session-<uuid>") to RESUME instead of creating a fresh session: the task is appended to that conversation and its previous context stays available. The session id of a preset_run-created session can be found as the newest "session-*" directory under ~/.dsh/sessions/<cwd>/ after the run (the child session).`
      },
      timeoutMs: {
        type: "number",
    ...
    async execute(args, exec) {
      const { preset, task, timeoutMs, sessionId } = args;
      ...
      try {
        if (sessionId) {
          // Resume: load the persisted session and append the task as the
          // next user message, keeping the previous conversation context
          // (same as the Web UI continuing an existing chat).
        ...
        throw new Error(`preset_run: failed to ${sessionId ? `resume session "${sessionId}"` : `create a "${preset}" session`}: ${error instanceof Error ? error.message : String(error)}`, { cause: error });
      }
      ...
      try {
        ...
        try {
          ...
          // Cleanup must never mask the child's own result/error (review feedback).
          console.warn(`preset_run: child session cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`);
        }
      }
    }
  }));
